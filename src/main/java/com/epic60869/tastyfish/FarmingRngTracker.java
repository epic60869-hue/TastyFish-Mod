...
/** TastyFish-owned port of the farming RNG detection behavior. */
public final class FarmingRngTracker {
    ...
    private static final Set<String> DROPS = Set.of(
        ...
        "Crystalized Moonlight", "Floral Gelatin", "Cropie", "Squash", "Fermento", "Burrowing Spores",
        "Overgrown Grass", "Green Bandana", "Dedication IV", "Dedication 4", "Flowering Bouquet", "Rooted Spores",
        "Fruit Bowl", "Atmospheric Filter", "Beady Eyes", "Clipped Wings", "Mantid Claw", "Wriggling Larva",
        "Locust Larva", "Squeaky Toy", "Squeaky Mousemat", "Vermin Vaporizer", "Synthesis", "Evergreen",
        "Quickdraw", "Hypercharge", "Fire in a Bottle", "Iridium", "Overclocker 3000", "Rabbit Hat",
        "Lucky Clover Core", "Bulky Stone", "Turbo-Wheat V", "Turbo-Carrot V", "Turbo-Potato V",
        "Turbo-Pumpkin V", "Turbo-Melon V", "Turbo-Cocoa V", "Turbo-Cactus V", "Turbo-Mushrooms V",
        "Turbo-Cane V", "Turbo-Warts V", "Cultivating X", "Cultivating 10", "Seasoning"
    );
    ...
    private void handle(Component message) {
        ...
        String raw = clean(message.getString()).trim();
        Parsed parsed = parse(raw);
        if (parsed == null) return;

        long unitPrice = "Seasoning".equalsIgnoreCase(parsed.name()) ? -1L : Math.round(ItemPriceResolver.value(toItemId(parsed.name())));
    ...
    private Parsed parse(String raw) {
        ...
        if (match.find()) item = match.group("item").trim();
            ...
            if (!raw.toLowerCase(Locale.ROOT).contains("rare drop") && !raw.toLowerCase(Locale.ROOT).contains("rngesus")) return null;
            Matcher older = OLDER_DROP.matcher(raw);
            if (!older.find()) return null;
            item = older.group("item").trim();
        }

        item = item.replaceAll("\\s*\\(\\+[^)]*\\)\\s*$", "").trim().replaceAll("!$", "").trim();
        int amount = 1;
    ...
    private static String toItemId(String name) {
        return name.toUpperCase(Locale.ROOT).replace(' ', '_').replace('-', '_')
            .replace("DEDICATION_IV", "DEDICATION_4")
            .replace("DEDICATION_V", "DEDICATION_5");
    }
