...
/** TastyFish-owned farming RNG detection. */
public final class FarmingRngTracker {
    ...
    public Drop active() {
        Drop current = active;
        if (current != null && System.currentTimeMillis() > current.shownUntil()) { active = null; return null; }
        return current;
    }
    ...
    private Parsed parse(String raw) {
        ...
        if (quantity.matches()) { amount = parseInt(quantity.group("x1"), quantity.group("x2")); item = quantity.group("item").trim(); }
    ...
    private static int parseInt(String first, String second) { try { return Integer.parseInt(first != null ? first : second); } catch (Exception ignored) { return 1; } }
    private static String rarity(String message) { String text = message.toLowerCase(Locale.ROOT); if (text.contains("rare crop")) return "RARE CROP"; if (text.contains("crazy rare")) return "CRAZY RARE"; if (text.contains("rngesus")) return "RNGESUS"; return "RARE DROP"; }
    private static String clean(String text) { return text.replaceAll("§[0-9a-fk-or]", ""); }
