...
public final class TastyFishScreen extends Screen {
    ...
    private TastyFishScreen(TastyFishConfig config, boolean editor) {
        super(Component.literal("TastyFish")); this.config = config; this.editor = editor;
    }
    ...
    @Override public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        if (editor && event.button() == 0) {
            if (isOverProfit(event.x(), event.y())) { dragging = "profit"; return true; }
            if (isOverRng(event.x(), event.y())) { dragging = "rng"; return true; }
        }
    ...
    @Override public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        if (editor && dragging != null && event.button() == 0) {
            ...
            if (dragging.equals("profit")) FarmingProfitHud.setPosition(nx, ny); else TastyFishRngHud.setPosition(nx, ny);
            return true;
        }
        return super.mouseDragged(event, dx, dy);
    }
    @Override public boolean mouseReleased(MouseButtonEvent event) {
        if (editor && event.button() == 0 && dragging != null) { dragging = null; return true; }
        return super.mouseReleased(event);
    }
    private boolean isOverProfit(double mx, double my) { return mx >= FarmingProfitHud.x() && mx <= FarmingProfitHud.x()+FarmingProfitHud.width() && my >= FarmingProfitHud.y() && my <= FarmingProfitHud.y()+FarmingProfitHud.height(); }
    private boolean isOverRng(double mx, double my) { return mx >= TastyFishRngHud.x() && mx <= TastyFishRngHud.x()+TastyFishRngHud.width() && my >= TastyFishRngHud.y() && my <= TastyFishRngHud.y()+TastyFishRngHud.height(); }
}
